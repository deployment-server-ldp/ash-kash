...
async function main() {
  ...
  await prisma.storeSetting.upsert({
    ...
    create: {
      ...
      footerAbout: "Ash & Kash is a premium women's fashion boutique offering elevated ready-to-wear, luxury pieces, and considered essentials.",
  ...
  // ── Roles & permissions (for the admin Users & Roles screen) ──────
  ...
  console.log(`Admin login: admin@ashkash.test / ${adminPassword}`);
  ...
  const brand = await prisma.brand.upsert({
    where: { slug: "ash-kash-atelier" },
    create: { name: "Ash & Kash Atelier", slug: "ash-kash-atelier", isActive: true },
    update: {},
  });
  ...
  const everyday = await prisma.collection.upsert({
    ...
    create: { name: "Everyday Essentials", slug: "everyday-essentials", description: "Wardrobe staples for daily wear.", imageUrl: PLACEHOLDER, type: "MANUAL", isActive: true, sortOrder: 0 },
    update: {},
  });
  ...
  const evening = await prisma.collection.upsert({
    ...
    create: { name: "Statement Evening Wear", slug: "statement-evening-wear", description: "Show-stopping pieces for special occasions.", imageUrl: PLACEHOLDER, type: "MANUAL", isActive: true, sortOrder: 1 },
    update: {},
  });
  ...
  const slider = await prisma.slider.upsert({
    ...
    create: { id: "seed-home-hero", name: "Homepage Hero", location: "home_hero", isActive: true },
    update: {},
  });
  const slideDefs = [
  ...
  // ── Homepage sections ──────────────────────────────────────────
  const sectionDefs: { type: Parameters<typeof prisma.homepageSection.create>[0]["data"]["type"]; title?: string; subtitle?: string; content?: string; buttonText?: string; buttonUrl?: string; settings?: object }[] = [
    { type: "HERO_SLIDER" },
  ...
  for (const c of customerDefs) {
    const user = await prisma.user.upsert({
      where: { email: c.email },
      create: { name: c.name, email: c.email, password: await bcrypt.hash("password123", 10), userRole: "CUSTOMER", isActive: true },
      update: {},
    });
    customers.push(user);
  ...
  console.log("Sample customer login: fatima@example.com / password123");
