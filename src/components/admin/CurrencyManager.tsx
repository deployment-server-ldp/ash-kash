...
function CurrencyRow({ currency }: { currency: Currency }) {
  ...
  return (
    <tr className="border-b border-stone/60">
      ...
      <td className="p-3">{Number(currency.exchangeRate)}</td>
...
function CurrencyFormFields({ currency, onDone }: { currency?: Currency; onDone: () => void }) {
  ...
  return (
    ...
    >
      ...
      <input type="number" step="0.000001" name="exchangeRate" defaultValue={currency?.exchangeRate?.toString() ?? "1"} placeholder="Rate" required className="input" />
